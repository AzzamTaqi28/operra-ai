...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${API_BASE_URL}/api/v1/auth/register-organization`, {
    method: "POST",
  ...
  if (!upstream.ok) {
    return NextResponse.json(payload ?? { error: { code: "INTERNAL_ERROR", message: "registration failed" } }, { status: upstream.status })
  }
